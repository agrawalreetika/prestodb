        ...
        extends AbstractTestQueryFramework
{
    ...
    public void testAsyncDataCacheCleanup() throws Exception
    {
        ...
        String cacheCleanupEndPoint = "/v1/operation/server/clearCache?name=hive&id=hive";
        ...
        sleep(120000); // Sleep to allow cache updates
        ...
        workerNodes.parallelStream().forEach(worker -> {
            int responseCode = sendWorkerRequest(worker.getInternalUri().toString(), cacheCleanupEndPoint);
    ...
    {
        String endPoint = "/v1/operation/server/clearCache?name=hive&id=hive";
        QueryRunner queryRunner = getQueryRunner();
        DistributedQueryRunner distributedQueryRunner = (DistributedQueryRunner) queryRunner;
        Set<InternalNode> workerNodes = getWorkerNodes(distributedQueryRunner);
        InternalNode workerNode = workerNodes.iterator().next();

        int responseCode = sendWorkerRequest(workerNode.getInternalUri().toString(), endPoint);
