        ...
        extends AbstractTestQueryFramework
{
    ...
    public void testAddColumnWithDefaultMultipleDataTypes()
    {
        ...
        try {
            assertUpdate(String.format("CREATE TABLE %s (id BIGINT) WITH (\"format-version\" = '3', format = 'PARQUET')", tableName));
            assertUpdate(String.format("INSERT INTO %s VALUES (1), (2), (3)", tableName), 3);
            // Add columns with different data types and default values
            ...
            assertUpdate(String.format("ALTER TABLE %s ADD COLUMN country VARCHAR DEFAULT 'IN'", tableName));
            // Verify historical rows return default values for all new columns
            ...
            // Insert new row without explicit values for default columns
            assertUpdate(String.format("INSERT INTO %s (id) VALUES (4)", tableName), 1);
            // New row should have NULL for columns (write-default not yet supported)
            ...
            // Insert new row with explicit values
