...
* GET: /v1/operation/server/clearCache?name=<connectorName>&id=<connectorId>: It clears the memory (and ssd) cache on worker node. Here is an example:

  .. sourcecode:: http

   curl -X GET "http://localhost:7777/v1/operation/server/clearCache?name=hive&id=hive"
