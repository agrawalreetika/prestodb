        ...
        extends IcebergDistributedTestBase
{
            ...
            throws Exception
    {
        ...
        //test invalidate_iceberg_manifest_file_cache procedure
        assertQuerySucceeds(session, format("CALL %s.system.invalidate_iceberg_manifest_file_cache()", catalogName));
        assertTrue(manifestFileCache.size() == 0);
        assertQuerySucceeds(session, "SELECT count(*) from test_manifest_file_cache group by i");
